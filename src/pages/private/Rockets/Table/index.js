...
export default function RocketsTable({ location: { state = {}, pathname } }) {
  ...
  const buyMatrix = () => {
    if (matrixInfo && matrixInfo.id) {
      ...
      api
        .buyMatrixRockets(Number(matrixInfo.id))
        .then(() => {
          setBuyingStatus({
            type: 'success',
          ...
          api
            .getMatrixRocketsStructureByType(matrixInfo.id)
            .then((response) => {
              if (response.items) {
  ...
  const closePartnerModal = () => {
    ...
    setTimeout(() => {
      // eslint-disable-next-line react/prop-types
      if (pathname.startsWith('/personal-rockets')) {
        api
          .getMatrixRocketsStructureByType(matrixInfo.id)
          .then((response) => {
            if (response.items) {
    ...
    if (matrixInfo && matricesList) {
      if (direction === 'left') {
        if (matrixInfo.id !== 1) {
          ...
          history.push(`/personal-rockets/${matrixInfo.id - 1}`)
        }
      } else {
        if (matrixInfo.id !== 8) {
  ...
  const handleUpMatrix = () => {
    if (id) {
      api
        .getUppeRocketsStructureById(id)
        .then((response) => {
          if (response.items) {
            setMatrixTree(response.items)
            setIsFetching(false)
  ...
  const backRouteElement = useMemo(() => {
    let returnRoute = (
      <Link to={routes.rockets} className={styles.close}>
  ...
  const navRoute = useCallback(
    (route = '') => {
      ...
      if (matrixInfo && matrixInfo.isActive && !id) {
        newRoute = `/personal-rockets/${matrixInfo.id}${route}`
      } else if (id) {
        newRoute = `/rocket/${id}${route}`
      }
      return newRoute
    },
    [matrixInfo, id],
    ...
    <div className={styles.Table}>
      <Container>
        <div className={styles.header}>
          ...
          {matrixInfo &&  <MyViewElement element={<h1 className={styles.title}>Rockets - {matrixInfo.name}</h1>}/>}
          {backRouteElement}
        </div>
          ...
          <div className={styles.content}>
            ...
            <div className={styles.matrixTree}>
              ...
              <div className={styles.secondRow}>
                {
                  matrixInfo.id === 1 || matrixInfo.id === 3
                    ?
                    <>
                      <MatrixCell
                        place={1}
