...
export default function ClonesModal({ status, matrixType, onClose }) {
  ...
  useEffect(() => {
    if (matrixType) {
      api
        .getMatrixMiniClonesCout(matrixType)
        .then((response) => {
          if (response.count > -1) {
                ...
                >
                  {() => (
                    <Form>
                      ...
                      <Button type="submit" disabled={callStatus || true} color="violet-blue">
                        Расставить
                      </Button>
                    </Form>
                  )}
