...
export default function IonTable({ location: { state = {}, pathname } }) {
  ...
  const buyMatrix = () => {
    if (matrixInfo && matrixInfo.id) {
      ...
      api
        .buyMatrixIon(Number(matrixInfo.id))
        .then(() => {
          setBuyingStatus({
            type: 'success',
          ...
          api
            .getMatrixIonStructureByType(matrixInfo.id)
            .then((response) => {
              if (response.items) {
  ...
  const closePartnerModal = () => {
    ...
    setTimeout(() => {
      // eslint-disable-next-line react/prop-types
      if (pathname.startsWith('/personal-ion')) {
        api
          .getMatrixIonStructureByType(matrixInfo.id)
          .then((response) => {
            if (response.items) {
    ...
    if (matrixInfo && matricesList) {
      if (direction === 'left') {
        if (matrixInfo.id !== 1) {
          ...
          history.push(`/personal-ion/${matrixInfo.id - 1}`)
        }
      } else {
        if (matrixInfo.id !== 8) {
  ...
  const backRouteElement = useMemo(() => {
    let returnRoute = (
      <Link to={routes.ion} className={styles.close}>
  ...
  const navRoute = useCallback(
    (route = '') => {
      ...
      if (matrixInfo && matrixInfo.isActive && !id) {
        newRoute = `/personal-ion/${matrixInfo.id}${route}`
      } else if (id) {
        newRoute = `/ions/${id}${route}`
      }
      return newRoute
    },
    [matrixInfo, id],
    ...
    <div className={styles.Table}>
      <Container>
        <div className={styles.header}>
          ...
          {matrixInfo &&  <MyViewElement element={<h1 className={styles.title}>Ion - {matrixInfo.name}</h1>}/>}
          {backRouteElement}
        </div>
