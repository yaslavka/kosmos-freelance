...
export default function KeplerTable({ location: { state = {}, pathname } }) {
  ...
  const closePartnerModal = () => {
    ...
    setTimeout(() => {
      // eslint-disable-next-line react/prop-types
      if (pathname.startsWith('/personal-sweeps')) {
        api
          .getMatrixStructureByType(matrixInfo.id)
          .then((response) => {
            if (response.items) {
    ...
    if (matrixInfo && matricesList) {
      if (direction === 'left') {
        if (matrixInfo.id !== 1) {
          ...
          history.push(`/personal-sweeps/${matrixInfo.id - 1}`)
        }
      } else {
        if (matrixInfo.id !== 8) {
  ...
  const navRoute = useCallback(
    (route = '') => {
      ...
      if (matrixInfo && matrixInfo.isActive && !id) {
        newRoute = `/personal-sweeps/${matrixInfo.id}${route}`
      } else if (id) {
        newRoute = `/sweeps/${id}${route}`
      }
      return newRoute
    },
    [matrixInfo, id],
