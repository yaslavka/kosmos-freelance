...
export default function RoyalsTable({ location: { state = {}, pathname } }) {
  ...
  const buyMatrix = () => {
    if (matrixInfo && matrixInfo.id) {
      ...
      api
        .buyMatrixRoyals(Number(matrixInfo.id))
        .then(() => {
          setBuyingStatus({
            type: 'success',
          ...
          api
            .getMatrixRoyalsStructureByType(matrixInfo.id)
            .then((response) => {
              if (response.items) {
  ...
  const closePartnerModal = () => {
    ...
    setTimeout(() => {
      // eslint-disable-next-line react/prop-types
      if (pathname.startsWith('/personal-royal')) {
        api
          .getMatrixRoyalsStructureByType(matrixInfo.id)
          .then((response) => {
            if (response.items) {
    ...
    if (matrixInfo && matricesList) {
      if (direction === 'left') {
        if (matrixInfo.id !== 1) {
          ...
          history.push(`/personal-royal/${matrixInfo.id - 1}`)
        }
      } else {
        if (matrixInfo.id !== 8) {
  ...
  const backRouteElement = useMemo(() => {
    let returnRoute = (
      <Link to={routes.royal} className={styles.close}>
  ...
  const navRoute = useCallback(
    (route = '') => {
      ...
      if (matrixInfo && matrixInfo.isActive && !id) {
        newRoute = `/personal-royal/${matrixInfo.id}${route}`
      } else if (id) {
        newRoute = `/royals/${id}${route}`
      }
      return newRoute
    },
    [matrixInfo, id],
    ...
    <div className={styles.Table}>
      <Container>
        <div className={styles.header}>
          ...
          {matrixInfo &&  <MyViewElement element={<h1 className={styles.title}>Royals - {matrixInfo.name}</h1>}/>}
          {backRouteElement}
        </div>
